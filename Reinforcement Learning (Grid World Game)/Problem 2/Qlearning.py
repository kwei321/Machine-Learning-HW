...
def main():
    ...
    # generate a random exploration matrix
#    exploratory_policy_matrix = np.random.randint(low=0, high=4, size=game.shape).astype(np.float32)
#    #change the wall to np.NaN
#    exploratory_policy_matrix = np.where(game == "wall",np.NaN,exploratory_policy_matrix).astype(np.float) 
#    #change the absorbing state to negative 1
#    for terminal_loc in terminal:
#        exploratory_policy_matrix[terminal_loc[0],terminal_loc[1]] = 1
    
    
#    # use optimal policy as exploratory policy
    exploratory_policy_matrix = np.array([
                  [-1, np.NaN, 1, 2, -1, -1, 1, -1],
                  [0, 3, np.NaN, 1, 1, 1, 1, 0],
                  [-1, np.NaN, 1, -1, 0, -1, 0, 0],
                  [1, 2, np.NaN, 1, 0, 3, -1, -0],
                  [np.NaN, 2, np.NaN,0, -1, -1, 0,np.NaN],
                  [-1, 1, 1, 0, 3, 3, -1, 3],
                  [1, 0, 0, 0, 3, np.NaN, 0, 0],
                  [0, -1, -1, 0, -1.00, 3, 3, 3],
                  ])   
